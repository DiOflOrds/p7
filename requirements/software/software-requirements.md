...
| SWR-073 | When the code on disk is newer than the running process (git HEAD of `platform` differs from the stand frozen at start, SWR-047) the server shall restart itself via the SWR-061 exit code — but only when it was launched by a relaunch loop (marker environment variable set by the start scripts), only after the new stand has been stable for a further check interval, and only while no request is being served; every automatic restart is announced on the console. Without the marker the behaviour is unchanged (banner + button only), so a server started by hand or by a test never exits on its own. The frontend shall detect the returning server and reload the page by itself. Operations CR from pm/N-0010, tracked as p7/T-0016 (pm/B013). | STK-017 | Unit tests (stand unchanged → no restart, changed+stable+idle → restart, missing marker → never, request in flight → postponed) + acceptance checklist (session changes code, server comes back on its own) | medium | reviewed |

## Traceability

STK-017 ← SWR-053–061, SWR-073 (complete; no orphans). v1.2: +SWR-060 (Betriebs-CR T-0014); v1.3: +SWR-061 (Betriebs-CR T-0015 aus pm/N-0002); v1.4: +SWR-073 (Betriebs-CR T-0016 aus pm/N-0010, Selbst-Neustart bei neuem Code). DoD checklist applied per SWR (2026-08-15 RM). Security: PIN read gate for team content (SWR-053) documented as ADR-006 delta in Sprint 1; Guardrail 2 (no GitHub remote for sensitive repos) untouched — Mission Control reads locally. G1 pending (T-0003).
